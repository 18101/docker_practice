...
下载好对应安装包后，如果要下载到C盘，直接双击 `Docker Desktop Installer.exe` 开始安装。

**如果要把程序装到C盘之外**：

1. 需要管理员权限PowerShell执行：
```powershell
Start-Process -Wait -FilePath "D:\Downloads\Docker Desktop Installer.exe" -ArgumentList 'install','-accept-license','--installation-dir="D:\Tools\Docker"'
## --installation-dir 就是程序安装目录。
```

2. 镜像容器数据迁移到其他盘

打开 Docker Desktop → 设置 ⚙️ → Resources → Advanced → Disk image location 点 Browse，选地址，例如 D:\DockerData。点击 Apply & Restart，自动迁移全部镜像容器数据。
